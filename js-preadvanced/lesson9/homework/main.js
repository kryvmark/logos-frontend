...
userForm.addEventListener('submit', (e) => {
    e.preventDefault();
    if (userForm.dataset.action == 'addUser')
        addUser();
    else if (userForm.dataset.action == 'editUser')
        saveEditUser();
    userForm.reset();
...
function addUser() {
    users.push(new User(formLogin(userForm.login.value), userForm.password.value, userForm.email.value, users.length + 1));
}
let userIndex = 0;
...
function saveEditUser() {
    const user = users[userIndex];
    user.login = formLogin(userForm.login.value);
